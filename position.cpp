...
std::vector<Move> Position::generateEvasionMoves()
{
    ...
    if (__builtin_popcount(control_dir_[~color_][king_sq_[color_]] & (DIR_MASK | JUMP_MASK)) == 1) {
        //8方向についてはループを回す
        for (int i = 0; i < 8; i++) {
    ...
    if (__builtin_popcount(control_dir_[~color_][king_sq_[color_]] & JUMP_MASK) == 1
        && __builtin_popcount(control_dir_[~color_][king_sq_[color_]] & DIR_MASK) == 0) {
        //周囲8方向を順番に見ていく	
        for (int i = 0; i < 8; i++) {
            //手番側の玉位置に飛び利きが立っているか
